...
const Projects = () => {
  ...
  const fetchProject = async () => {
    ...
    try {
      ...
      console.log("Current Code:", data.project?.current_code);
      console.log("Conversations:", data.project?.conversation?.length);
  ...
  const togglePublish = async () => {
    try {
      const { data } = await api.patch(`/api/user/publish-toggle/${projectId}`);
  ...
  }, [projectId, session?.user, project?.current_code]);
      ...
      <div className="flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar">
        ...
        <div className="flex items-center gap-2 sm:min-w-90 text-nowrap">
          ...
          <div className="sm:hidden flex-1 flex justify-end">
            {isMenuOpen ? <MessageSquareIcon onClick={() => setMenuOpen(false)} className="size-6 cursor-pointer" /> : <XIcon onClick={() => setMenuOpen(true)} className="size-6 cursor-pointer" />}
          </div>
        </div>
